-- First ensure Steve has the correct organization ID in metadata
UPDATE auth.users
...
-- Re-seed food relationships data
DO $$
DECLARE
...
BEGIN
  ...
  DELETE FROM food_sub_categories WHERE organization_id = org_id;
  DELETE FROM food_categories WHERE organization_id = org_id;
  DELETE FROM food_category_groups WHERE organization_id = org_id;
  ...
  -- Update any existing master ingredients to use new category IDs
  UPDATE master_ingredients
  SET 
    major_group = food_group_id,
    category = proteins_cat_id
  WHERE organization_id = org_id
  AND (product ILIKE '%beef%' OR product ILIKE '%pork%' OR product ILIKE '%chicken%');
